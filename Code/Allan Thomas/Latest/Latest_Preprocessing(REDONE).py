...
# MSI categorical: missing rows get their own "MISSING" label so the model
#   can learn from the absence of MSI data rather than ignoring it.
# Numeric columns (ploidy_wes, ploidy_snp6, mutational_burden): converted to
#   float and filled with the overall column median. Missingness is small
#   enough (<1%) that row deletion would be wasteful.
df6 = df5.copy()
...
# CHOOSE FINAL FEATURE SET  (built from df6)
...
if "DRUG_NAME" in X_train.columns:
    ...
    X_train["DRUG_NAME_target_enc"] = X_train["DRUG_NAME"].map(
        y_train.groupby(X_train["DRUG_NAME"]).mean()
