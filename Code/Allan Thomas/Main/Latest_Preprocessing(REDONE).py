...
# ----- Stage 3: UNCLASSIFIED resolution -----
...
# TARGET is removed because of substantial missingness and fine-grained sparsity.
# TARGET_PATHWAY is the stable, mechanism-level feature kept for modeling.
# df5 is the final modeling-ready dataframe used for everything downstream.
...
# CHOOSE FINAL FEATURE SET  (built from df5)

target_col = "LN_IC50"

id_cols = ["COSMIC_ID", "CELL_LINE_NAME", "DRUG_ID"]
id_cols = [col for col in id_cols if col in df5.columns]
...
candidate_feature_cols = [col for col in candidate_feature_cols if col in df5.columns]

X_candidate = df5[candidate_feature_cols].copy()
y = df5[target_col].copy()
modeling_df = df5[id_cols + candidate_feature_cols + [target_col]].copy()
...
# All cleaning, filling, and TCGA remapping is done. TARGET has been dropped.
# No encoding or scaling has been applied yet — still fully human-readable.
df5.to_csv("FINAL_PREPROCESSED_DF.csv", index=False)
print("\nSaved: FINAL_PREPROCESSED_DF.csv —", df5.shape)


# TRAIN / TEST SPLIT  (from df5 via X_candidate)
...
# Columns to one-hot encode — TARGET removed, TARGET_PATHWAY kept
categorical_cols = [
    "DRUG_NAME",
    "TCGA_DESC",
    "GDSC Tissue descriptor 1",
