...
# Combine: OHE columns + numeric columns (includes DRUG_NAME_target_enc)
...
# This section standardizes the continuous numeric features using a scaler fit 
# only on the training data so scale-sensitive models can compare them fairly
# Scaled columns: the three continuous genomic features + DRUG_NAME_target_enc,
# which is also a continuous float (mean LN_IC50 per drug) and needs the same
# treatment. OHE binary columns are left unchanged.
...
'''
# DROP TCGA UNCLASSIFIED ROWS
# These rows were found to add noise in earlier draft modeling, so remove them
# before building the final modeling dataset.

if "TCGA_DESC" in df6.columns:
    before_rows = len(df6)
    unclassified_rows = (df6["TCGA_DESC"].astype("string").str.strip() == "UNCLASSIFIED").sum()

    df6 = df6.loc[
        df6["TCGA_DESC"].astype("string").str.strip() != "UNCLASSIFIED"
    ].copy()

    after_rows = len(df6)

    print("\nDropped UNCLASSIFIED TCGA rows:")
    print("  Rows removed :", int(unclassified_rows))
    print("  Rows before  :", before_rows)
    print("  Rows after   :", after_rows)
    print("  UNCLASSIFIED remaining :",
          int((df6["TCGA_DESC"].astype("string").str.strip() == "UNCLASSIFIED").sum()))
...
def objective(trial):
    ...
    trial.set_user_attr("mae", -scores["test_neg_mean_absolute_error"].mean())
    return -scores["test_neg_mean_squared_error"].mean()
...
print("RMSe= ", (study.best_value) ** 0.5)
print("Corresponding R²:", study.best_trial.user_attrs["r2"])
print("Corresponding MAE:", study.best_trial.user_attrs["mae"])


#Generalized Cross Validation to estimate an optimal alpha for Ridge regression
ridge_model = Ridge()
alphas = np.logspace(-3, 3, 100)  # Test alphas from 0.001 to 1000
param_grid = {"alpha": alphas}
ridge_cv = RandomizedSearchCV(
    estimator=ridge_model,
    param_distributions=param_grid,
    n_iter=20,  # Number of different alphas to try
...
#makes histograms of the target variable (LN_IC50) in the training 
# set to visualize its distribution and check for skewness or outliers before modeling.
import matplotlib.pyplot as plt

plt.figure()
plt.hist(y_train, bins=30)
plt.title("Target Distribution")
plt.xlabel("Drug Sensitivity")
